...
const SPEECH_SEND_DELAY_MS = 1000;
...
async function startListening() {
    ...
    try {
        ...
        microphoneSource = audioContext.createMediaStreamSource(sttStream);
        microphoneSource.connect(analyser);
        
        const mimeType = MediaRecorder.isTypeSupported('audio/webm') ? 'audio/webm' : '';
        mediaRecorder = new MediaRecorder(sttStream, mimeType ? { mimeType } : undefined);
        audioChunks = [];
        ...
        showToast('Microphone access denied or error. Check permissions.');
        if (autoListenMode) {
...
function stopListening() {
    isSpeaking = false; // Prevent sending to STT if stopped manually
    stopListeningInternal();
}
...
async function processSTT(blob) {
    if (speechWidget) speechWidget.classList.add('visible');
    if (speechWidgetText) speechWidgetText.textContent = 'Thinking...';
    ...
    try {
        ...
        if (speechWidgetText) speechWidgetText.textContent = 'Audio processing error.';
        setTimeout(() => {
            if (speechWidget && !isListening) speechWidget.classList.remove('visible');
        }, 2000);
    }
...
function startCamera(facingMode = 'user') {
    ...
    if (camStream && currentFacingMode !== facingMode) {
    ...
    return navigator.mediaDevices.getUserMedia({ video: { facingMode: currentFacingMode }, audio: false })
        .then(stream => {
            camStream = stream;
...
function setMode(mode) {
    ...
    [btnJarvis, btnGeneral, btnRealtime].forEach(b => b?.classList.remove('active'));
    ...
    if (currentMode === 'jarvis') {
        ...
        if (modeSlider) modeSlider.classList.add('center');
