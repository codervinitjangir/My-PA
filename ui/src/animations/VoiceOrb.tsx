...
// WebGL Renderer Class ported from vanilla JS
class OrbRenderer {
  container: HTMLElement;
  hue: number;
  hoverIntensity: number;
  bgColor: number[];
  targetHover: number = 0;
  ...
  _compile(type: number, src: string) {
  ...
  _loop(ts: number) {
    ...
    if (this.u.iResolution) gl.uniform3f(this.u.iResolution, this.canvas.width, this.canvas.height, this.canvas.width / this.canvas.height);
    if (this.u.hue) gl.uniform1f(this.u.hue, this.hue);
    if (this.u.hover) gl.uniform1f(this.u.hover, this.currentHover);
    if (this.u.rot) gl.uniform1f(this.u.rot, this.currentRot);
    if (this.u.hoverIntensity) gl.uniform1f(this.u.hoverIntensity, this.hoverIntensity);
    if (this.u.backgroundColor) gl.uniform3f(this.u.backgroundColor, this.bgColor[0], this.bgColor[1], this.bgColor[2]);
...
export const VoiceOrb: React.FC<VoiceOrbProps> = ({ isActive = false, size = 200, className = '' }) => {
  ...
  useEffect(() => {
    ...
    orbRef.current = new OrbRenderer(containerRef.current, {
      hue: 0,
      hoverIntensity: 0.3,
      backgroundColor: [0.039, 0.039, 0.039], // matched to zinc-950 (approx)
    });
  ...
  useEffect(() => {
    if (orbRef.current) {
      orbRef.current.setActive(isActive);
    }
  }, [isActive]);

  return (
    <div 
      className={`relative flex items-center justify-center transition-all duration-500 ease-in-out ${isActive ? 'opacity-100 scale-105 drop-shadow-[0_0_40px_rgba(124,106,239,0.5)]' : 'opacity-35 scale-100'} ${className}`}
