...
// LiveKit controller that synchronizes the voice agent state with the 3D Orb
function LiveKitVoiceController({ sceneRef, setVoiceState }: { sceneRef: React.RefObject<OrbSceneApi | null>, setVoiceState: (s: string) => void }) {
  const { state } = useVoiceAssistant();
  
  useEffect(() => {
    setVoiceState(state.toUpperCase());
    if (state === "listening") {
      sceneRef.current?.zoomIn();
    } else if (state === "speaking") {
      sceneRef.current?.rotateBy(0.5, 0.5);
    } else if (state === "disconnected" || state === "idle") {
      sceneRef.current?.resetView();
    }
  }, [state, sceneRef, setVoiceState]);
...
export default function JarvisOrb() {
  ...
  const [voiceState, setVoiceState] = useState<string>("CONNECTING...");
  ...
  useEffect(() => {
    ...
    const fetchToken = () => {
      ...
      const baseUrl = typeof window !== 'undefined' ? `${window.location.protocol}//${host}:8000` : "http://127.0.0.1:8000";
      fetch(`${baseUrl}/api/livekit/token`)
        ...
        .catch(err => {
          console.warn("Failed to fetch LiveKit token, retrying in 2s...", err.message);
          if (mounted) setTimeout(fetchToken, 2000);
        });
    };
    fetchToken();
  ...
  return (
    <>
      ...
      <LiveKitRoom
        serverUrl={lkUrl}
        token={lkToken}
        connect={!!lkToken && !!lkUrl}
        audio={true}
        style={{ display: "none" }}
      >
        <LiveKitVoiceController sceneRef={sceneRef} setVoiceState={setVoiceState} />
      </LiveKitRoom>

      <div className="hud hud-title">J.A.R.V.I.S. <br/><span style={{fontSize: "0.5em", color: "#00D9FF"}}>{voiceState}</span></div>
