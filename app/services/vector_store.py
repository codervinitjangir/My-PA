...
class VectorStoreService:
        ...
        for file_path in sorted(LEARNING_DATA_DIR.glob("*.txt")):
            ...
            try:
                ...
                if content:
                    ...
                    logger.info(f"[VECTOR] Loaded learning data: %s (%d chars)", file_path.name, len(content))
    ...
    def create_vector_store(self) -> FAISS:
        ...
        if not all_documents:
            ...
            logger.info(f"[VECTOR] Split into %d chunks (chunk_size=%d, overlap=%d)",
                        len(chunks), CHUNK_SIZE, CHUNK_OVERLAP)

            self.vector_store = FAISS.from_documents(chunks, self.embeddings)
            logger.info(f"[VECTOR] FAISS index built successfully with %d vectors", len(chunks))
