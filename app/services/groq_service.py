...
class GroqService:
    ...
    def _invoke_llm(
        ...
        for j in range(n):
            ...
            keys_tried.append(i)
            masked_key = mask_api_key(GROQ_API_KEYS[i])
        ...
        masked_all = ", ".join([mask_api_key(GROQ_API_KEYS[i]) for i in keys_tried])
                    ...
                    if isinstance(content, str) and content:
                        ...
                        if len(accumulated) - last_check_len >= REPEAT_CHECK_INTERVAL:
                            ...
                            if _detect_repetition_loop(accumulated):
                                logger.warning(f"[STREAM] Repetition loop detected after %d chars - stopping", len(accumulated))
    ...
    def _build_prompt_and_messages(
        ...
        try:
            ...
            if context_docs:
                ...
                logger.info(f"[CONTEXT] Retrieved {len(context_docs)} chunks from sources: {list(set(context_sources))}")
            else:
